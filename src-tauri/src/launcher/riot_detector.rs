//! Riot Games detector - finds Valorant, League of Legends, and other Riot games
//!
//! SAFETY: ALL operations are READ-ONLY
//! - Registry: Uses only open_subkey() and get_value() - never writes
//! - Files: Uses only read_to_string(), read_dir(), exists() - never writes
...
/// Configuration for Riot Games titles
#[derive(Debug, Clone)]
pub struct RiotGameConfig {
    pub name: &'static str,
    pub folder_name: &'static str,
    pub game_exe_path: &'static str,  // Relative path to game exe (for detection)
    pub icon_exe_path: Option<&'static str>,  // Optional: separate exe for icon extraction (if different from game_exe)
    #[allow(dead_code)]
    pub process_name: &'static str,   // Process name for playtime tracking
    pub product_id: &'static str,     // Riot Client product ID for launching
    pub patchline: &'static str,      // Patchline (usually "live")
}
...
impl RiotGameConfig {
    pub const VALORANT: RiotGameConfig = RiotGameConfig {
        ...
        icon_exe_path: Some(r"live\VALORANT.exe"),  // Use launcher for icon (has HD icon, game exe doesn't)
    ...
    pub const LEGENDS_OF_RUNETERRA: RiotGameConfig = RiotGameConfig {
        ...
        product_id: "bacon",  // Riot's internal codename for LoR
        patchline: "live",
    };
    ...
    /// Get the launch arguments for Riot Client
    pub fn get_launch_args(&self) -> String {
        format!("--launch-product={} --launch-patchline={}", self.product_id, self.patchline)
    }
}

/// Detect all installed Riot Games
/// SAFETY: ALL operations are READ-ONLY - no registry writes, no file modifications
///
/// Riot Games must be launched through the Riot Client with specific arguments.
/// This detector finds the Riot Client and sets up proper launch_args for each game.
#[cfg(windows)]
pub fn detect_riot_games() -> Vec<DetectedGame> {
    let mut games = Vec::new();

    // Find all Riot Games installation paths
    let riot_paths = find_riot_games_paths();

    for riot_path in riot_paths {
        // Find Riot Client in this Riot Games folder
        // Riot Client is at: <Riot Games>\Riot Client\RiotClientServices.exe
        let riot_client_path = riot_path.join("Riot Client").join("RiotClientServices.exe");

        // SAFETY: exists() is READ-ONLY
        if !riot_client_path.exists() {
            // No Riot Client found in this path, skip
            continue;
        }

        let riot_client_str = riot_client_path.to_string_lossy().to_string();

        // Scan for games in this Riot Games folder
...
/// Find product icon from Riot Client's metadata/cache
...
fn find_riot_product_icon(riot_path: &std::path::Path, product_id: &str, cache_dir: &std::path::Path) -> Option<String> {
    ...
    // Priority 1: Check ProgramData metadata folder (official Riot icon location)
    // Path: C:\ProgramData\Riot Games\Metadata\{product_id}.live\{product_id}.live.ico
    ...
    // Priority 2: Riot Client stores product icons in various locations
    // Check Riot Client's UX folder for product icons
    let icon_search_paths = [
        // Riot Client UX resources
    ...
    // Icon filename patterns to look for
    ...
    for search_path in &icon_search_paths {
        ...
        if let Ok(entries) = fs::read_dir(search_path) {
            for entry in entries.flatten() {
                ...
                // Check if filename matches any pattern or contains product_id
...
/// Find and copy .ico file from game folder to cache
/// SAFETY: READ-ONLY - only reads directory and copies file
fn find_ico_in_folder(game_folder: &std::path::Path, cache_dir: &std::path::Path, product_id: &str) -> Option<String> {
    use std::fs;

    // Look for .ico files in the game folder and immediate subdirectories
    let search_paths = [
        ...
        if let Ok(entries) = fs::read_dir(search_path) {
            for entry in entries.flatten() {
                ...
                if path.extension().map(|e| e.to_ascii_lowercase() == "ico").unwrap_or(false) {
                    // Found an .ico file, copy it to cache
                    let output_path = cache_dir.join(format!("riot_{}.png", product_id));
                    ...
                    // Try to copy/convert the ico to png using PowerShell
                    if let Some(icon_path) = convert_ico_to_png(&path, &output_path) {
                        return Some(icon_path);
                    }
                }
...
/// Convert .ico file to .png using PowerShell - extracts the LARGEST icon from the .ico file
/// SAFETY: READ-ONLY on input, writes only to cache directory
#[cfg(windows)]
fn convert_ico_to_png(ico_path: &std::path::Path, output_path: &std::path::Path) -> Option<String> {
    use std::process::Command;

    // This script extracts the largest icon from an .ico file
    // ICO files contain multiple sizes (16x16, 32x32, 48x48, 256x256, etc.)
    // 256x256 icons are typically stored as embedded PNG data which we extract directly
    // Smaller icons use BMP format and are extracted via System.Drawing.Icon
...
/// Find all Riot Games installation paths using multiple detection methods
/// SAFETY: ALL methods are READ-ONLY - no registry writes, no file modifications
#[cfg(windows)]
fn find_riot_games_paths() -> Vec<PathBuf> {
    let mut paths = Vec::new();

    // Priority 1: Read Riot Client configuration
    // SAFETY: READ-ONLY - only reads config files
    paths.extend(find_riot_from_config());

    // Priority 2: Registry detection
    // SAFETY: READ-ONLY - only reads registry keys
    paths.extend(find_riot_from_registry());

    // Priority 3: Folder scanning
...
/// Find Riot Games from Riot Client configuration files
/// SAFETY: READ-ONLY - only uses fs::read_to_string()
#[cfg(windows)]
fn find_riot_from_config() -> Vec<PathBuf> {
    let mut paths = Vec::new();

    // Check %LocalAppData%\Riot Games\RiotClientInstalls.json
    if let Some(local_data) = dirs::data_local_dir() {
        let riot_config = local_data.join("Riot Games").join("RiotClientInstalls.json");

        // SAFETY: read_to_string is READ-ONLY
        if let Ok(content) = std::fs::read_to_string(&riot_config) {
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
                // Look for rc_default or rc_live paths
                for key in ["rc_default", "rc_live", "associated_client"] {
                    if let Some(path_obj) = json.get(key) {
                        // Can be a string or an object with paths
    ...
    // Check %ProgramData%\Riot Games\RiotClientInstalls.json
    if let Some(program_data) = std::env::var_os("PROGRAMDATA") {
...
/// Extract Riot Games root folder from a path to RiotClientServices.exe
/// SAFETY: Pure path manipulation, no I/O
fn extract_riot_root_from_path(path: &str) -> Option<PathBuf> {
    let path = PathBuf::from(path);

    // Path might be to RiotClientServices.exe, we want the "Riot Games" folder
...
fn find_riot_from_registry() -> Vec<PathBuf> {
    ...
    // Also check direct Riot Games registry key
...
/// Find Riot Games root from an install path (could be a game subfolder)
/// SAFETY: Pure path manipulation + exists() check
fn find_riot_root_from_install(install_path: &str) -> PathBuf {
    ...
    // Return original path if we can't find "Riot Games" parent
    path
}

/// Find Riot Games from common folder locations
/// SAFETY: READ-ONLY - only uses path.exists() and path.join()
#[cfg(windows)]
fn find_riot_from_folders() -> Vec<PathBuf> {
    ...
    for letter in b'C'..=b'Z' {
        ...
        for path in possible_paths {
            // SAFETY: exists() and is_dir() are READ-ONLY
...
mod tests {
    ...
    fn test_find_riot_root_from_install() {
        // Test that we can find Riot Games folder from a game path
        let path = r"E:\Entertainment\Riot Games\VALORANT\live";
        let result = find_riot_root_from_install(path);
        // Since the path doesn't exist, it returns the original
